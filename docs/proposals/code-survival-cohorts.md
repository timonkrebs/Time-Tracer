...
| Truncated/malformed patch trusted        | `applyPatch` checks each hunk consumed/produced exactly its header's `oldCount`/`newCount`, returning `null` (→ blob fallback) on any mismatch.                                                                                                                      |
| Undated commit skipped                   | An unparseable date no longer skips the commit (which drifted the snapshot); it borrows the monotonic clock and still applies.                                                                                                                                       |
...
| Generated/vendored files skew the charts | The walk now applies `isGeneratedFile` (lockfiles, `dist/`, minified bundles …), holding them out of the lifetimes exactly as the rest of Insights does.                                                                                                             |
| Pure copies recorded as empty            | A `copied` change with no content change fetches the copied blob instead of returning the empty target snapshot, so its lines count as new births.                                                                                                                   |
| Mass-move of identical files             | Rename sources are matched through a **per-content queue**, so moving many byte-identical files in one commit pairs each with its own target instead of dropping all but the last.                                                                                   |
| Rename that overwrites a file            | A `git mv -f old existing` (removed source + modified target with the source's content) carries the source's age/author into the target and retires the target's original lines.                                                                                     |
| Rename into an ignored path              | A tracked file moved into a generated/ignored path (`git mv src/app.ts dist/app.js`, or a provider rename whose new path is ignored) **censors** the source's lines and vacates the old path — never false deaths, never left alive on a path that no longer exists. |
| No-op "modified" overwrite               | An overwrite-rename target must show a real content change, so a mode-only `modified` duplicate can't be paired with a deleted twin and swap its age/author.                                                                                                         |
